<div class="flex h-[calc(100vh-4rem)]">
    <div class="flex-1 flex flex-col bg-zinc-100 dark:bg-zinc-900">
        ...
        <div class="flex space-x-2 mb-4 p-1 overflow-x-auto">
            @foreach(['Menu Nasi', 'Orak-Arik', 'Nasi Goreng', 'Menu Mie', 'bubur', 'Snack', 'Tambahan', 'Minuman'] as $cat)
                <button
        ...
        <div class="p-5">
            ...
            <div class="space-y-6">
                @foreach($itemVariants as $group)
                    <div>
                        <h3 class="font-semibold text-sm text-zinc-800 dark:text-zinc-200 mb-3">
                            {{ $group['group_name'] }}
                            @if($group['track_stock']) 
                                <span class="ml-1 text-[10px] font-bold text-amber-500 bg-amber-100 dark:bg-amber-900/30 px-2 py-0.5 rounded-full">Pakai Stok</span> 
                            @endif
                        </h3>
                        <div class="flex flex-wrap gap-2">
                            @foreach($group['options'] as $opt)
                                @php
                                    $isSelected = isset($selectedOptions[$group['group_id']]) && $selectedOptions[$group['group_id']] === $opt['id'];
                                    $isOutOfStock = $group['track_stock'] && $opt['stock'] <= 0;
                                @endphp
                                <button
                                    wire:click="$set('selectedOptions.{{ $group['group_id'] }}', {{ $opt['id'] }})"
                                    @disabled($isOutOfStock)
                                    class="px-4 py-2 border-2 rounded-xl text-sm font-medium transition-all text-left
                                        {{ $isSelected ? 'border-indigo-600 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/40 dark:border-indigo-500 dark:text-indigo-300' : 'border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:border-indigo-300' }}
                                        {{ $isOutOfStock ? 'opacity-40 cursor-not-allowed bg-zinc-50 dark:bg-zinc-800/50' : '' }}"
                                >
                                    {{ $opt['name'] }}
                                    @if($group['track_stock'])
                                        <span class="block mt-1 text-[10px] font-bold {{ $opt['stock'] > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500' }}">
                                            Stok: {{ $opt['stock'] }}
                                        </span>
                                    @endif
                                </button>
            ...
            <div class="mt-8 flex justify-end gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                ...
                <flux:button wire:click="confirmVariantSelection" variant="primary" class="bg-indigo-600 hover:bg-indigo-700" 
                    :disabled="count($selectedOptions) !== count($itemVariants)">
                    Tambah ke Keranjang
                </flux:button>
            </div>
        </div>
